...
my $BuzzDeskUser = 'buzzdesk';    # default: buzzdesk
...
my ( $Help, $DryRun, $SkipArticleDir, @SkipRegex, $BuzzDeskUserID, $WebGroupID, $AdminGroupID );

sub PrintUsage {
    print <<EOF;

Set BuzzDesk file permissions.

Usage:
 buzzdesk.SetPermissions.pl [--buzzdesk-user=<BUZZDESK_USER>] [--web-group=<WEB_GROUP>] [--admin-group=<ADMIN_GROUP>] [--skip-article-dir] [--skip-regex="REGEX"] [--dry-run]

Options:
 [--buzzdesk-user=<BUZZDESK_USER>]   - BuzzDesk user, defaults to 'buzzdesk'.
 ...
 [--skip-regex="REGEX"]        - Add another skip regex like "^/var/my/directory". Paths start with / but are relative to the BuzzDesk directory. --skip-regex can be specified multiple times.
...
sub Run {
    ...
    # check params
    $BuzzDeskUserID = getpwnam $BuzzDeskUser;
    if ( !$BuzzDeskUser || !defined $BuzzDeskUserID ) {
        print STDERR "ERROR: --buzzdesk-user is missing or invalid.\n";
        exit 1;
    }
...
sub SetPermissions {
    ...
    # Make sure it is inside the BuzzDesk directory to avoid following symlinks outside
    if ( substr( $File, 0, $FrameworkDirectoryLength ) ne $FrameworkDirectory ) {
        $File::Find::prune = 1;    # don't descend into subdirectories
        return;
    }

    # Now get a canonical relative filename under the BuzzDesk directory
...
sub SetFilePermissions {
    ...
    # Writable by default, owner BuzzDesk and group webserver.
    my ( $TargetPermission, $TargetUserID, $TargetGroupID ) = ( 0660, $BuzzDeskUserID, $WebGroupID );
    if ( -d $File ) {

        # SETGID for all directories so that both BuzzDesk and the web server can write to the files.
