...
print STDOUT "\nManage the BuzzDesk daemon process.\n\n";

local $Kernel::OM = Kernel::System::ObjectManager->new(
    'Kernel::System::Log' => {
        LogPrefix => 'BuzzDesk-buzzdesk.Daemon.pl',
    },
);
...
if ( $CurrentUser ne $ApplicationUser ) {
    print STDERR
        "Error: You cannot run buzzdesk.Daemon.pl as user $CurrentUser. Please run it as user $ApplicationUser or with the help of su:\n";
    print STDERR "  su -c \"bin/buzzdesk.Daemon.pl ...\" -s /bin/bash $ApplicationUser\n";
    exit 1;
}
...
sub PrintUsage {
    my $UsageText = "Usage:\n";
    $UsageText .= " buzzdesk.Daemon.pl action [--debug] [--force] [--foreground]\n";
    $UsageText .= "\nOptions:\n";
    ...
    $UsageText .= "\n buzzdesk.Daemon.pl start --debug SchedulerTaskWorker SchedulerCronTaskManager\n\n";
    $UsageText
        .= "Forced stop reduces the time the main daemon waits other daemons to stop from normal 30 seconds to 5.\n";
    $UsageText .= "\n buzzdesk.Daemon.pl stop --force\n";
    ...
    while ($DaemonChecker) {
        ...
        for my $Module ( sort keys %DaemonModules ) {
            ...
            if ( !$ChildPID ) {
                ...
                local $Kernel::OM = Kernel::System::ObjectManager->new(
                    'Kernel::System::Log' => {
                        LogPrefix => "BuzzDesk-buzzdesk.Daemon.pl - Daemon $Module",
                    },
                );
...
sub _LogFilesSet {
    ...
    # get log rotation type and backup old logs if logs should be rotated by BuzzDesk
    my $RotationType = lc $ConfigObject->Get('Daemon::Log::RotationType') || 'buzzdesk';

    # if rotation type is OTRS, set to BuzzDesk
    if ( $RotationType eq 'otrs' ) {
        $RotationType = 'buzzdesk';
    }

    if ( $RotationType eq 'buzzdesk' ) {
    ...
    return 1 if $RotationType ne 'buzzdesk';

    # remove not needed log files if BuzzDesk rotation is enabled
...
sub _LogFilesCleanup {
    ...
    # skip cleanup if BuzzDesk log rotation is not enabled
    my $RotationType = lc $Kernel::OM->Get('Kernel::Config')->Get('Daemon::Log::RotationType') || 'buzzdesk';
