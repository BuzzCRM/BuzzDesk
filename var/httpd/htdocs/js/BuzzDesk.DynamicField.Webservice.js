...
BuzzDesk.DynamicField = BuzzDesk.DynamicField || {};

/**
 * @namespace
 * @exports TargetNS as BuzzDesk.DynamicField.Webservice
 * @description
 *      This namespace contains the special functions for dynamic field types WebserviceDropdown and WebserviceMultiselect.
 */
BuzzDesk.DynamicField.Webservice = (function (TargetNS) {
    ...
    TargetNS.InitSelect = function (InputFieldUUID, DynamicFieldName, SelectedValueFieldName, AutocompleteFieldName, AutocompleteMinLength, QueryDelay, DefaultSearchTerm, TicketID, AdditionalDFs) {
        ...
        function AutocompleteSelect($AutocompleteField, SearchTerm) {
            ...
            Core.AJAX.FunctionCall(
                ...
                function (Response) {
                    ...
                    // BuzzDesk.Form.Input.Set does not support this.
                    // BuzzDesk.Form.Input.Set(SelectedValueFieldName, Options, {SelectOption: true, Modernize: true, TriggerChange: true, AddEmptyOption: true});
                    SetInputFieldSelectableOptions(Options, $AutocompleteField);

                    if (!jQuery.isEmptyObject(SelectedIDs)){

                        // BuzzDesk.Form.Input.Set(SelectedValueFieldName, SelectedIDs);
        ...
        function SetInputFieldSelectableOptions(Options, $AutocompleteField) {
            ...
            Core.App.Publish('BuzzDesk.Form.Input.Change.' + DynamicFieldName);
        ...
        $('#' + SelectedValueFieldName).off('change.Multiselect').on('change.Multiselect', function () {
            var SelectedIDs = BuzzDesk.Form.Input.Get(SelectedValueFieldName),
                InitAutoFill;

            if (SelectedIDs && AdditionalDFs && AdditionalDFs.length){
                $.each(AdditionalDFs, function(Index, DynamicField) {
                    var Exists = BuzzDesk.Form.Input.Exists('DynamicField_' + DynamicField);
            ...
            function (Response) {
                ...
                $.each(Response, function(DynamicFieldName, Value) {
                    var DynamicField = 'DynamicField_' + DynamicFieldName,
                        Exists = BuzzDesk.Form.Input.Exists(DynamicField);

                    if (Exists){
                        BuzzDesk.Form.Input.Set(DynamicField, Value);
                    }
                });
            },
            'json'
    ...
    return TargetNS;
}(BuzzDesk.DynamicField.Webservice || {}));
