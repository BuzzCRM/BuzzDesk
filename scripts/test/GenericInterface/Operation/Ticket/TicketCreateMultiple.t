...
my @Tests        = (
    ...
    {
        Name           => 'Ticket with multiple articles',
        SuccessRequest => 1,
        SuccessCreate  => 1,
        RequestData    => {
            Ticket => {
                    ...
                    Attachment => [
                        {
                            Content     => 'VGhpcyBpcyBhIHRlc3QgdGV4dC4=',
                            ContentType => 'text/plain; charset=us-ascii',
                            Filename    => '1.ArticleAttachment',
                            Disposition => 'attachment',
                        },
                    ],
                },
                {
                    Subject                         => '2. Article subject',
                    Body                            => '2. Article body',
                    AutoResponseType                => 'auto reply',
                    ArticleTypeID                   => 1,
                    SenderTypeID                    => 1,
                    ...
                    Attachment => [
                        {
                            Content     => 'VGhpcyBpcyBhIHRlc3QgdGV4dC4=',
                            ContentType => 'text/plain; charset=us-ascii',
                            Filename    => '2.ArticleAttachment 1',
                            Disposition => 'attachment',
                        },
                        {
                            Content     => 'VGhpcyBpcyBhIHRlc3QgdGV4dC4=',
                            ContentType => 'text/plain; charset=us-ascii',
                            Filename    => '2.ArticleAttachment 2',
                            Disposition => 'attachment',
                        },
                    ],
                },
...
for my $Test (@Tests) {
    ...
    if ( $Test->{SuccessCreate} ) {
        ...
        my @RequesterResultArticleIDs = $RequesterResult->{Data}->{ArticleID};
        if ( IsArrayRefWithData( $RequesterResult->{Data}->{ArticleID} ) ) {
            @RequesterResultArticleIDs = @{ $RequesterResult->{Data}->{ArticleID} };
        }
        my @LocalResultArticleIDs = $LocalResult->{Data}->{ArticleID};
        if ( IsArrayRefWithData( $LocalResult->{Data}->{ArticleID} ) ) {
            @LocalResultArticleIDs = @{ $LocalResult->{Data}->{ArticleID} };
        }

        my $Counter = 0;
        for my $ArticleID (@RequesterResultArticleIDs) {

            my $RequesterResultArticleID = $ArticleID;
            my $LocalResultArticleID     = $LocalResultArticleIDs[$Counter];

            my $LocalArticleBackendObject = $ArticleObject->BackendForArticle(
                TicketID  => $LocalResult->{Data}->{TicketID},
                ArticleID => $LocalResultArticleID,
            );
            ...
            my $RequesterArticleBackendObject = $ArticleObject->BackendForArticle(
                TicketID  => $RequesterResult->{Data}->{TicketID},
                ArticleID => $RequesterResultArticleID,
            );
            ...
            for my $Attribute (qw(Subject Body ContentType MimeType Charset From)) {
                ...
                if (
                    IsArrayRefWithData( $Test->{RequestData}->{Article} )
                    && $Test->{RequestData}->{Article}->[$Counter]->{$Attribute}
                    )
                {
                    $Self->Is(
                    ...
                    IsHashRefWithData( $Test->{RequestData}->{Article} )
            ...
            for my $Attribute (qw(SenderType)) {
                if ( IsArrayRefWithData( $Test->{RequestData}->{Article} ) ) {

                    if ( $Test->{RequestData}->{Article}->[$Counter]->{ $Attribute . 'ID' } ) {
                        $Self->Is(
                            $LocalArticleData{ $Attribute . 'ID' },
                            $Test->{RequestData}->{Article}->[$Counter]->{ $Attribute . 'ID' },
                ...
                elsif ( IsHashRefWithData( $Test->{RequestData}->{Article} ) ) {
            ...
            if ( $Test->{RequestData}->{Attachment} && ref $Test->{RequestData}->{Attachment} eq 'HASH' ) {
                push @RequestedAttachments, $Test->{RequestData}->{Attachment};
            }
            if ( $Test->{RequestData}->{Attachment} && ref $Test->{RequestData}->{Attachment} eq 'ARRAY' ) {
                @RequestedAttachments = @{ $Test->{RequestData}->{Attachment} };
            }

            if ( $Test->{RequestData}->{Attachment} ) {
                $Self->IsDeeply(
                    \@Attachments,
