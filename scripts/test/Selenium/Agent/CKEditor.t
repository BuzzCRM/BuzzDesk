    ...
    sub {
        ...
        my @TestCasesBasic = (
            ...
            },
            {
                ...
                'Expected' =>
                    '<p>Here is an allowed element with a forbidden attribute: <strong>Strong text</strong>.</p>',
            },
        );
        ...
        $Selenium->WaitFor(
            JavaScript =>
                "return typeof(\$) === 'function' && \$('.ck-editor__editable_inline').contents().length == 1"
        );
       ...
       # send some text to the CKE's textarea (we cant do it with Selenium directly because the textarea is not visible)
        ...
        $Selenium->WaitFor(
            JavaScript => "return window.editor.getData() ",
        );
        ...
        for my $TestCase (@TestCasesBasic) {
            ...
            $Selenium->execute_script( 'window.editor.setData("' . $TestCase->{Input} . '");' );
