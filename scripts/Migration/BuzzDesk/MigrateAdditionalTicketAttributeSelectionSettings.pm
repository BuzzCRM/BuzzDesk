...
package scripts::Migration::BuzzDesk::MigrateAdditionalTicketAttributeSelectionSettings;    ## no critic
...
These settings were added in BuzzDesk 7.3 to allow for the selection of additional ticket attributes.
...
Returns the SysConfig settings to be migrated.
    ...
    my %MigrateSysConfigSettings = $MigrateToBuzzDeskObject->_GetMigrateSysConfigSettings();
...
sub Run {
    ...
    my $PackageIsInstalled1 = $PackageObject->PackageIsInstalled(
        Name => 'BuzzDesk4OTRS-AdditionalTicketAttributeSelection',
    );

    my $PackageIsInstalled2 = $PackageObject->PackageIsInstalled(
        Name => 'BuzzDesk-AdditionalTicketAttributeSelection',
    );

    if ( !$PackageIsInstalled1 && !$PackageIsInstalled2 ) {
        print "\n\n Package BuzzDesk(4OTRS)-AdditionalTicketAttributeSelection is not installed. Skipping migration.\n";
        return 1;
    }
