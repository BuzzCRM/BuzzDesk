...
package Kernel::Output::HTML::ToolBar::TicketProcessCategory;
...
sub Run {
    ...
    my $ProcessCategoryExists;
    PROCESS:
    for my $Process ( sort keys %{$ProcessList} ) {
        my %ProcessPreferences = $ProcessObject->ProcessPreferencesGet(
            ProcessEntityID => $Process,
            UserID          => $Self->{UserID},
        );
        if ( $ProcessPreferences{Category} ) {
            $ProcessCategoryExists = 1;
            last PROCESS;
        }
    }

    return if !$ProcessCategoryExists;
