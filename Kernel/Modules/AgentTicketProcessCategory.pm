...
package Kernel::Modules::AgentTicketProcessCategory;
...
sub Run {
    ...
    # Get the list of processes that agent can start
...
sub _DisplayProcessCategory {
    ...
    my $Config             = $ConfigObject->Get('Ticket::Frontend::AgentTicketProcessCategory');
    ...
    for my $ProcessData (@Process) {
        ...
        # Set the LinkTarget from the Process if it is set (ProcessPreferences) or from
        # the `Ticket::Frontend::AgentTicketProcessCategory` Config
        ...
        if ( $LinkTarget eq 'AsPopup' ) {
            ...
            $Link = $LayoutObject->{Baselink}
                . "Action=AgentTicketProcess;Subaction=DisplayActivityDialog;ProcessEntityID="
    ...
    $Output .= $LayoutObject->Output(
        TemplateFile => 'AgentTicketProcessCategory',
        Data         => {
            %Param,
