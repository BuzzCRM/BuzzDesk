...
package Kernel::System::SupportDataCollector::Plugin::BuzzDesk::TimeSettings;
...
sub GetDisplayPath {
    return Translatable('BuzzDesk') . '/' . Translatable('Time Settings');
}
...
sub Run {
    ...
    if ( defined $OTRSTimeZone ) {
        $Self->AddResultOk(
            ...
            Label      => Translatable('BuzzDesk time zone'),
            Value      => $OTRSTimeZone,
        );
    }
    else {
        $Self->AddResultProblem(
            ...
            Label      => Translatable('BuzzDesk time zone'),
            Value      => '',
            Message    => Translatable('BuzzDesk time zone is not set.'),
        );
    }
